...
export default function NewReservationPage() {
  ...
  const onSubmit = async (data: CreateReservationInput) => {
    try {
      ...
      // 백엔드 API 필드명에 맞춰 데이터 매핑
      await createReservationMutation.mutateAsync({
        companyId: data.operatorId,
        serviceType: data.serviceType,
        requestedDate: reservationDateTime,
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <Card>
          ...
          <CardContent className="space-y-6">
            ...
            {/* 차량 번호 입력 */}
            <div className="space-y-2">
              <Label htmlFor="vehicleInfo">차량 번호 (선택)</Label>
              <Input
                id="vehicleInfo"
                type="text"
              ...
              <p className="text-xs text-gray-500">
                한국 차량 번호판 형식으로 입력하세요 (예: 12가3456, 123나7890)
              </p>
            </div>
